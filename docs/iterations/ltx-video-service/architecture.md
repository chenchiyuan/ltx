...
| Phase 2: GPU Execution | 1 台 8 卡 GPU 服务器跑满 8 个可配置 Worker 并接入真实执行 | `gpu_server/` 部署子项目、8 Worker、Worker Registry、ComfyUI Worker、ComfyUI-LTXVideo、DCGM/Worker 指标、GPU E2E | Phase 1 API、任务状态机、工作流版本、资产存储适配器不变 |
...
- Phase 2 首版以 1 台 8 卡 GPU 服务器、8 个可配置 Worker 为验收口径。
- Phase 2 首版部署入口是 `gpu_server/`；单机 8 Worker 可先用 Docker Compose + NVIDIA Container Toolkit，Kubernetes/k3s + NVIDIA GPU Operator 保留为多机/Phase 2.5 演进路径。
...
| GPU Server 子项目 | `gpu_server/` 中提供镜像、8 Worker 编排、模型下载、健康检查、部署说明 | F-005, F-007, F-012 | NEW | 新增 |
...
| Docker Compose + NVIDIA Container Toolkit | Phase 2 单机 8 Worker 首版部署编排 | F-005, F-007, F-012 | NEW | 新增 |
...
- Phase 2 首版通过 `gpu_server/` 提供 Docker Compose 8 Worker 部署；Kubernetes/GPU Operator 作为多机或 Phase 2.5 演进路径。
- KEDA 只在 Scale 阶段启用。Phase 2 接入 GPU 后，Worker 常驻更重要，因为 LTX 模型加载成本高。

### 3.5 `gpu_server/` 部署子项目边界

`gpu_server/` 是 Phase 2 GPU 节点的独立部署入口，不是临时脚本目录。目标效果是：在 GPU 服务器 clone 仓库后，进入 `gpu_server/` 即可构建或拉取镜像，并启动 8 个可配置 Worker。
...
- 默认 `WORKER_COUNT=8`，可配置为 1 到 GPU 数量。
- 每个 Worker 固定一个 `CUDA_VISIBLE_DEVICES`，一张 GPU 同时只跑一个业务 attempt。
...
| GPU Server 子项目 | NEW | `gpu_server/` 提供镜像、8 Worker 编排、模型脚本、健康检查和部署说明 | GPU 运行时 | 高 |
...
| GPU 单机编排 | NEW | Docker Compose + NVIDIA Container Toolkit 首版跑 8 Worker；K8s/GPU Operator 后置 | 部署运维 | 中 |
...
| GPU Server | Worker 数量配置、GPU 绑定、模型路径校验 | compose 8 Worker 启动 | 单机 8 Worker 全部注册并可被摘除 |
...
- Phase 2 接入单机静态 GPU 容量池：一张 GPU = 一个执行槽位。
- Phase 2 首版一台 8 卡服务器默认部署 8 个 GPU Worker，`WORKER_COUNT` 可配置。
- 单任务单卡，不做多卡切分。
